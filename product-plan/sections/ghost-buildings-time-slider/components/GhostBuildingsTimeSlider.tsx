...
export function GhostBuildingsTimeSlider({
  ...
  const visibleGhostCount = Object.values(ghostVisibility).filter(Boolean).length
  ...
  const ghostPositions = useMemo(() => {
    const allLats = ghostBuildings.flatMap((g) => g.parcelBoundary.map((p) => p[1]))
    const allLngs = ghostBuildings.flatMap((g) => g.parcelBoundary.map((p) => p[0]))
    ...
    <div className="flex h-full" style={{ fontFamily: '"Inter", system-ui, sans-serif' }}>
      ...
      <div className="flex-1 relative bg-[#0F0F22] overflow-hidden select-none">
        ...
        <div
          ...
          style={{
            background: 'radial-gradient(ellipse at center, transparent 25%, rgba(10,5,5,0.6) 100%)',
          }}
        />
        ...
        >
          ...
          >
            ...
            <div
              ...
              style={{
                background: 'linear-gradient(135deg, rgba(20,15,30,0.9), rgba(10,8,20,0.95))',
            ...
            {/* Ghost blocks */}
            {ghostLayerVisible && ghostPositions.map(({ ghost, left, top, width }) => (
              <GhostBlock
                key={ghost.id}
                ghost={ghost}
        ...
        <div className="absolute top-4 left-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              >
                {visibleGhostCount} of {ghostBuildings.length} structures demolished
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="absolute top-4 right-4 flex items-center gap-2">
          ...
          >
            <div
              ...
              style={{
                border: '1.5px dashed',
                borderColor: ghostLayerVisible ? 'rgba(244,67,54,0.5)' : 'rgba(100,116,139,0.4)',
        ...
        {hoveredId && hoveredGhost && !selectedId && ghostVisibility[hoveredId] && (
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div
                    ...
                    style={{ fontFamily: '"IBM Plex Mono", monospace' }}
                  >
                    Demolished {hoveredGhost.yearDemolished} · {hoveredGhost.demolitionCause.replace('-', ' ')}
                  </div>
                </div>
              </div>
            </div>
