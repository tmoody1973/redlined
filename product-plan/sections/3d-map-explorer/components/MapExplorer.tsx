...
export function MapExplorer({
  ...
  const zonePositions = useMemo(() => {
    ...
    return holcZones.map((zone) => {
      ...
      const lngSpan =
        Math.max(...zone.polygon.map((p) => p[0])) -
        Math.min(...zone.polygon.map((p) => p[0]));
      const latSpan =
        Math.max(...zone.polygon.map((p) => p[1])) -
        Math.min(...zone.polygon.map((p) => p[1]));
    ...
    <div className="absolute inset-0 bg-[#1A1A2E] overflow-hidden select-none">
      ...
      >
        ...
        >
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(135deg, rgba(30,30,60,0.8), rgba(15,15,35,0.9))",
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div
                      ...
                      style={{ fontFamily: '"IBM Plex Mono", monospace' }}
                    >
                      {zone.holcId} · {formatCurrency(zone.medianIncome)} median
                      income
                    </div>
                  </div>
                </div>
      ...
      <div className="absolute bottom-5 right-5 flex items-center gap-4">
        <div className="flex items-center gap-1.5">
          ...
          >
            <path
              d="M15 3h4a2 2 0 012 2v4M9 21H5a2 2 0 01-2-2v-4M21 9v6M3 9v6"
              strokeLinecap="round"
